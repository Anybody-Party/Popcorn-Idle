...
public class PanAndZoom : MonoBehaviour
{
    ...
    private static PanAndZoom _instance;
    ...
    public static PanAndZoom Instance // Init not in order
    {
        get
        {
            if (_instance == null)
    ...
    {
        _instance = FindObjectOfType<PanAndZoom>();
    }
    #endregion
    ...
    public void Attach(Transform _tr)
    {
        cinemachine = _tr;
    }
    ...
    void OnPinch(Vector2 center, float oldDistance, float newDistance, Vector2 touchDelta)
    {
        ...
        if (controlCamera && cameraControlEnabled)
        {
            ...
            if (cam.orthographic)
            {
                ...
                if (isCinemachineUse)
                    cinemachine.transform.position -= newPinchPosition - currentPinchPosition;
                else
